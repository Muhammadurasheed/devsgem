...
const Deploy = () => {
  ...
  useEffect(() => {
    const successMsg = messages.find(m => m.deploymentUrl || m.content.includes('Live at:'));
    if (successMsg) {
      setDeploymentUrl(successMsg.deploymentUrl || successMsg.content.match(/https:\/\/[^\s]+/)?.[0]);
    }
  }, [messages]);

  // Check if we should show deployment actions (e.g., "Would you like to deploy?")
  useEffect(() => {
  ...
  const handleProceedDeployment = () => {
    ...
    // Set deployment starting in UI
    setShowDeployActions(false);

    // Centralized handler handles naming prompt interception
    handleActionClick({
      id: 'deploy-confirm',
      label: 'Deploy',
      type: 'button',
    ...
    toast.success('Deployment sequence initiated!');
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <Card className="border-border/50 bg-background/50 backdrop-blur">
            ...
            <CardContent>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 h-[2px] bg-border" />
                <div className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${deploymentUrl ? 'bg-green-500 text-white' : (messages.length > 0 || activeDeployment) ? 'bg-yellow-500 text-white' : 'bg-muted text-muted-foreground'}`}>
                    {deploymentUrl ? <CheckCircle2 className="w-5 h-5" /> : (messages.length > 0 || activeDeployment) ? <Loader2 className="w-5 h-5 animate-spin" /> : '3'}
                  </div>
                  <span className={`text-sm ${deploymentUrl || messages.length > 0 || activeDeployment ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>
                    Deploy
                  </span>
                </div>
              </div>
          ...
          {/* Repository Selector */}
          {isGitHubConnected && !selectedRepo && (!activeDeployment || activeDeployment.status === 'success' || activeDeployment.status === 'failed') && (
            <RepoSelector onSelectRepo={handleSelectRepo} />
          )}

          {/* Deployment Progress & Results */}
          {(selectedRepo || (activeDeployment && activeDeployment.status !== 'success' && activeDeployment.status !== 'failed')) && (
            <>
              <DeploymentProgress
                messages={messages}
                isTyping={isTyping}
              ...
              <CardContent className="space-y-3 text-sm text-muted-foreground">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p>DevGem analyzes, builds, and deploys to *.servergem.app - no Google Cloud setup needed!</p>
                  </div>
                </div>
              </CardContent>
            </Card>
