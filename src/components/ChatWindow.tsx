...
const ChatWindow = ({ onClose, initialMessage }: ChatWindowProps) => {
  ...
  useEffect(() => {
    ...
    // Trigger confetti if latest message has a deployment URL
    const lastMessage = messages[messages.length - 1];
    if (lastMessage?.deploymentUrl && lastMessage?.role === 'assistant') {
      triggerConfetti();
    }
  }, [messages]);
  ...
  useEffect(() => {
    ...
    const resize = (e: MouseEvent) => {
      ...
      const newWidth = Math.max(240, Math.min(450, e.clientX - 20)); // Approximate relative to viewport
                        ...
                        <div className="flex-1 min-w-0">
                          {editingSessionId === session.id ? (
                            <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                              ...
                              />
                              <button onClick={() => handleRenameSubmit(session.id)} className="text-green-500">
          ...
          <div className="flex items-center gap-1">
            ...
            {messages.length > 0 && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 hover:bg-white/10 rounded-lg transition-all text-[#8b5cf6] font-bold text-[10px] uppercase tracking-wider group mr-1"
                title="Start a new thread (Clear context)"
              >
                <PlusCircle size={14} className="group-hover:rotate-90 transition-transform" />
                New Thread
              </button>
                    ...
                    <div key={message.id}>
                      <ChatMessage
                        ...
                        onServiceNameSubmit={(name) => {
                          sendStructuredMessage('service_name_provided', {
                            name,
                            repo_url: message.metadata?.repo_url
                          });
                        }}
                      />
                      ...
                      {message.deploymentUrl && !message.metadata?.hideLegacyUrl && (
                        <div className="flex items-center gap-2 mb-4 ml-12">
                          <code className="flex-1 px-3 py-2 bg-[#101827]/80 border border-white/5 rounded-lg text-sm text-[#06b6d4] font-mono truncate">
                            {message.deploymentUrl}
                          </code>
                          <button
                            onClick={() => copyToClipboard(message.deploymentUrl!)}
                            className="p-2 bg-accent/20 hover:bg-accent/40 rounded-lg transition-colors border border-accent/20"
                            aria-label="Copy URL"
                          >
                            {copiedUrl ? (
